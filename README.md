...
민간 마일즈전투 경연대회 참가팀을 위한 **분대 전술 상황판** 안드로이드 앱.

무전만으로는 지금 어느 방면이 무너졌는지 알기 어렵습니다. TacMap KCTC는 분대별 생존 인원,
아군의 실시간 위치, 거점 점령 상태를 지도 한 장에 모아, 화면을 잠깐 보는 것만으로
전황을 파악할 수 있게 만든 앱입니다.

**제3회 민간 마일즈전투 경연대회(K-ICTC)** 참가팀이 쓰기 위해 만들었습니다.
육군 과학화전투훈련단이 주최하는 민간인 대상 대회로, 김부리 도시지역훈련장의
A · B 경기장에서 팀당 15명이 점령전과 섬멸전을 치릅니다. 경기당 15분씩 리그전과 토너먼트로 진행되며,
앱의 라운드 초기화 기능은 이 짧은 경기 주기에 맞춰 만들었습니다.

> 민간 참가자를 위한 대회이며, 이 앱은 참가팀이 자체적으로 쓰기 위해 만든 개인 프로젝트입니다.
> 지도는 OpenStreetMap 계열의 공개 지도 서비스만 사용합니다.
...
APK 파일은 팀 단톡방이나 공유 링크로 배포합니다. 받은 링크를 눌러 **`TacMap_KCTC.apk`** 를 내려받으세요.

"이런 유형의 파일은 기기에 피해를 줄 수 있습니다" 라는 경고가 뜨면 **다운로드**를 누릅니다.
스토어를 거치지 않은 앱이라 나오는 안내이며, 파일 자체에는 문제가 없습니다.

> 저장소 접근 권한이 있다면 [Releases 페이지](../../releases/latest)에서 직접 받을 수도 있습니다.
...
| **알림** | 다른 앱(디스코드 등)을 화면에 띄운 동안에도 위치 전송을 유지하기 위해 |

위치 권한은 **"앱 사용 중에만 허용"** 으로 충분합니다.
실수로 거부했다면 `설정 → 애플리케이션 → TacMap KCTC → 권한` 에서 다시 켤 수 있습니다.
...
1. 앱을 열고 **등번호 · 이름 · 소속대학**을 입력합니다. 한 번 넣으면 다음부터 기억합니다.
...
![위성 지도](docs/screenshot-satellite.jpg)
...
디스코드 등 음성 앱을 화면에 띄워 둔 상태에서도 위치 전송이 유지됩니다.
...
- **라운드 초기화** — 한 경기가 끝나면 전원 사망 표시를 풀고, 적 표식과 기록을 지우고,
  ...
  경기당 15분씩 여러 판을 치르는 대회 방식에 맞춰 만든 기능입니다
- **전적 보기** — 분대별 · 개인별 사망 횟수와 거점 상태 변경 횟수를 집계합니다. 경기 후 팀원들과 돌아볼 때 씁니다
...
- **필드 프리셋** — 대회 전에 잡아둔 거점 · 리스폰 · 경계 배치를 저장해 두었다가,
  당일 새로 만든 방에 그대로 불러옵니다. 대회 공지가 나오면 미리 세팅해 두는 용도입니다

거점 점령 상태는 방장이 아니어도 누구나 바꿀 수 있습니다.
현장에서 무전으로 듣고 HQ가 대신 갱신하는 경우가 많기 때문입니다.

---

## 4. 지도 선택

배경 지도는 다섯 가지 중에 고를 수 있습니다.

| 지도 | 쓰임새 |
|---|---|
| **등고선** (기본) | 능선과 계곡을 읽어야 하는 산악 지형 |
| Esri 위성 | 건물과 도로를 눈으로 확인 |
| 구글 위성 / 위성+지명 | 위성 사진 대안 |
| 일반 지도 | 도로 위주의 단순한 화면 |

![등고선 지도](docs/screenshot-topo.jpg)

국내 지도 서비스는 가려지는 구역이 있어, 해외 지도 제공처를 선택해 쓸 수 있도록 했습니다.
...
/rooms/{6자리코드}
  meta          : 방장 uid, 진행 상태
  teams/{id}    : 분대 이름, 색, 순서
  members/{uid} : 등번호, 이름, 소속대학, 분대, 방장 · 관전 여부
...
구글 타일은 비공식 엔드포인트를 사용하므로 내부 확인 용도로만 두었으며 상업적으로 사용하지 않습니다.
