...
1. 이 페이지 오른쪽(모바일에서는 아래쪽)의 **Releases** 를 누릅니다.
   바로 가려면 → **[Releases 페이지](../../releases/latest)**
2. 가장 위에 있는 최신 버전에서 **`TacMap_KCTC.apk`** 를 누르면 다운로드가 시작됩니다.
3. "이런 유형의 파일은 기기에 피해를 줄 수 있습니다" 라는 경고가 뜨면 **다운로드**를 누릅니다.
...
| 표시 | 의미 |
|---|---|
| 색 원 + 부채꼴 | 분대원의 위치와 바라보는 방향. 색은 소속 분대 |
| 회색 원 + ✕ | 사망 처리된 분대원 |
...
국내 지도 서비스는 군사 지역을 가리기 때문에, 해외 지도 제공처를 선택해 쓸 수 있도록 했습니다.
...
| 지도 | flutter_map + 교체 가능한 타일 소스 | API 키와 과금 없이 무료 범위에서 운영 |
...
/rooms/{6자리코드}
  meta          : 방장 uid, 진행 상태
  teams/{id}    : 분대 이름, 색, 순서
  members/{uid} : 등번호, 이름, 소속대학, 분대, 방장 여부
